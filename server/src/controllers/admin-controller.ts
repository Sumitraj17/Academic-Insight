...
export const getAllAdmins = async (
    ...
    try {
        const [admins] = await connection.promise().query<Admin[]>("SELECT * FROM teacher WHERE isAdmin = true");
...
export const getAllTeachers = async (
    ...
    try {
        const [teachers] = await connection.promise().query<Teacher[]>("SELECT * FROM teacher WHERE isAdmin = false");
...
export const getAllRecords = async (
    ...
    try {
        const [records] = await connection.promise().query("SELECT * FROM Admin_display");
...
export const adminLogin = async (
    ...
    try {
        ...
        return res.status(201).json({ message: "OK", name: existingAdmin[0].teacher_Name, email: existingAdmin[0].email });
    } catch (error) {
        console.log(error);
...
export const verifyAdmin = async (
    ...
    try {
        ...
        return res.status(200).json({ message: "OK", name: admin[0].teacher_Name, email: admin[0].email });
    } catch (error) {
        console.log(error);
        return res.status(200).json({ message: "ERROR", cause: error.message });
    }
